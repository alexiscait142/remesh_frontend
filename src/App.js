...
class App extends Component {
  ...
  searchConversations = () => {
    return this.state.conversations.filter(convo=>{
      return convo["title"].toLowerCase().includes(this.state.searchTerm.toLowerCase())
    })
  }
  ...
  showMessages = () => {
    return this.state.findMessages.map(message => {
      return <div className="one-message">
              <p>{message["text"]}</p>
              <p>Date/Time created: {message["date_time_created"]}</p>
              <p>Thoughts:</p>
              {message["thoughts"].map(thought => {
                return <p className="message-thought">{thought["text"]}, {thought["date_time_created"]}</p>
                })
              }
              <button onClick={this.handleClick} className="message-button" value={message["id"]}>Add Thought</button>
              {this.state.showForm ? <Form message={this.state.messageID} postThought={this.addThought}/> : null}
            </div>
    })
  }
  ...
  render(){
    return (
      <div className="App">
        <input className="searchbar" placeholder="search conversations.." onChange={this.handleChange}></input>
        {this.showConversations()}
        <div className="messages-div">
          {this.state.showMessages ? this.showMessages() : null}
          {this.state.showMessages ? <button 
                                        onClick={this.handleClick}
